...
- Let's test this:
  #+begin_src C :results silent
    ...
    height = 8; // definition (aka assignment)
  #+end_src
...
- Phew! The next block finally works, that is, it compiles and
  ...
  #+begin_src C :results silent
    ...
    height = 8;
    height = 80000;
    height = 8.0;  // no complaints from the compiler but chaos will ensue
  #+end_src
  #+begin_quote
...
- A /floating-point literal/ assigned to a ~float~ variable contains a
  ...
  #+begin_src C :results silent
    float profit;
    profit = 2150.48f; // the `f` is optional but kind
  #+end_src
...
* PRACTICE  Variable Manipulation and Formatted Output (Canvas)

Create a =NEW= file!

   1. Declare an integer variable named =num= and initialize it to 10.
   2. Declare an integer variable named =doubleNum= that holds twice the
      value of =num=.
   3. Declare a floating-point variable named =half= that holds half of
      the value of =num=.
   4. Print the result on three lines using a single =printf= statement:
      #+begin_example
      The number is: 10
