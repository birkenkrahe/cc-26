...
#+options: toc:nil num:nil ^:nil date:nil author:nil
...
If you have Emacs + Org-mode, you can do this effortlessly (but you
have to know your way around Emacs + Org-mode). You can create new
code blocks (like in Google Colab) and run them without having to
enter =#include ... return 0= every time.

If you only have a browser, you have choices as explained in the
...
Give it a suitable title like ="Variables - Practice"=, write a short
description, like:
#+begin_quote
...
#+end_example
Make sure the visibility is =Public (visible to everyone)= so that you
can post the link to Canvas if requested.
