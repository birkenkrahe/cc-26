...
- Examples:
  ...
  + *String manipulation*: Strings are arrays (working with text -
    ...
    #+begin_src C
      ...
      printf("Hello, %s\n", name); // the whole string = array of chars
    #+end_src
...
- Imagine you have a /pointer-to-~int~ variable/ ~myPointer~ that contains
  the address of the variable ~iResult~:
  #+begin_src C
    int iResult = 75;
    printf("%d <- %p\n", iResult, &iResult);
...
- Solution:
  ...
  #+RESULTS:
  : 0x7ffd909f44fc 0x7ffd909f44fc 7
...
- What happens here?
  #+begin_src C :main no :includes
    ...
    void swap(int a, int b) {
      int temp; // store value of a temporary variable
      temp = a;
      a = b;
      b = temp;
...
- Observation:
  #+begin_quote
  Though we swapped the values in the function, they are not swapped
  in the main program.
  #+end_quote  
...
* PRACTICE Incrementing an integer by value/reference
...
- Solution:
  #+begin_src C :main no :includes
    ...
    { // BEGIN
      ...
      // PRINT "Sum with pointer: " + addPtr(num)
      printf("Sum with pointer: %d\n", addPtr(num)); // same as addPtr(&num[0])
      return 0;
    } // END
...
- Notice that we did not pass the size of the array to the function:
  we did not need this because we did not loop over the array.
...
* PRACTICE Passing arrays and pointers to functions
...
- Notice that we did not pass the size of the array to the function:
  Instead we used the constant =N= which was set with ~#define~.
