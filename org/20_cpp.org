...
- Open ~onecompiler.com/cpp~ - this will bring up the template program
  ...
  #+begin_src C++ :main no :includes :results output
    ...
    {
      cout << "Hello, C++ world!"; // direct the output to stdout
      printf("\n%s\n","Hello, C world!"); // C functions work, too
...
- It's just as (deceptively) simple to enter user input in
  ...
  ~STDIN~ field.
  #+begin_src cpp :tangle ../src/cin.cpp :main no :inludes :noeval :exports both
    #include <iostream>
    #include <string>
    using namespace std;
    ...
    {
      string name;
      cout << "What's your name? "; // ask user
      cin >> name; // get input
      cout << name << endl; // print input
      cout << "Hello, " << name << "!\n"; // print greeting
      return 0;
    }
  #+end_src
...
- It should look like this now (with your name):
...
- In other words, everyone can mess with the =Player= data.
...
* How to access ~private~ data - =getHealth()=
  ...
  - Overview:
    ...
    + You can also say that the ~private~ data are what a ~class~ has got
      (its *features* or attributes), and the ~public~ methods are its
      *behavior* - what it can *do*.
    #+end_quote

  - You've already met one one those methods: =move_point= for the =Point=
    structure, passing an address received by a pointer to =Point p=.
    #+begin_example C
...
- In the ~main~ program, we're adding a =Player= who can take damage:
  #+begin_src C++ :noweb yes :includes <iostream> :namespaces std :results output
    ...
    // John's health before the fight:
    cout << "John's health before the fight = " << John.getHealth() << endl;

    // In a fight, John takes damage
    John.takeDamage(50);

    // John's health after the fight:
    cout << "John's health after the fight = " << John.getHealth() << endl;
  #+end_src

  #+RESULTS:
  : John's health before the fight = 100
  : John's health after the fight = 50

* PRACTICE Heal the =Player= with =heal= (DIY challenge)
...
- Here is the starter code: https://tinyurl.com/heal-the-player
  #+begin_src C++ :main no :includes :results output
...
* PRACTICE Cap =Player= =health= at 100
...
- Starter code: [[https://onecompiler.com/cpp/43h3n5mgm][onecompiler.com/cpp/43h3n5mgm]],
  [[https://tinyurl.com/heal-the-player-2][tinyurl.com/heal-the-player-2]]
...
- Starter code: [[https://onecompiler.com/cpp/43gpntxa2][onecompiler.com/cpp/43gpntxa2]]
  https://tinyurl.com/player-with-constructor
...
- We add a =Player= =level= to the ~private~ data:
...
- Any number of members can be initialized, e.g. this list would
  construct a =Player= with default =health=100=, =level=1=, and =build=3=:
  #+begin_example C++
  Player() : health(100), level(1), build(3) { }
  #+end_example

- To create a =Player= with that constructor, in ~main~:
  #+begin_example C++
    Player Jane; // invokes `Player()` in the class
  #+end_example

- Why is this constructor list initialization better?
...
* PRACTICE BankAccount With Initializer Lists

- Write a C++ program that defines a ~BankAccount~ class.

- The class must have three *private* data members:
  1. ~balance~
  2. ~owner~
  3. ~accountNumber~ (must not change after initialization)

- The class must provide:
  1. A constructor that uses an *initializer list* to initialize all
     members.
  2. A public method ~printAccount()~ that prints account details.

** Starter code

https://onecompiler.com/cpp/44mxmm498

https://tinyurl.com/bank-account-initializer
...
int main() {
  BankAccount a1("Alice", 500.0, 1001);
  BankAccount a2("Bob", 1200.5, 1002);
...
#+begin_example
Account 1001 owned by Alice has balance 500
Account 1002 owned by Bob has balance 1200.5
#+end_example
...
3. Do not assign values inside the constructor body.
...
#+begin_src C++ :results output
  ...
  public:
    ...
    BankAccount(string o, double b, int n)
      : owner(o), balance(b), accountNumber(n) {}
    void printAccount() {
      // print account details
...
* PRACTICE Create two =Player= characters
...
- Example: Here, the ~class~ =Player= has a member variable =health= (full
  ...
  #+begin_src C++ :main no :includes :results output
    ...
    public:
      Player(int h) {
        this->health = h; // LHS: ptr to member, RHS: parameter
      }
...
- The Starter code is here:
  ...
  https://onecompiler.com/cpp/43h48uajv

  https://tinyurl.com/this-pointer
...
- Starter code:
  [[https://onecompiler.com/cpp/43h3pgda6][onecompiler.com/cpp/43h3pgda6]]
  
  https://tinyurl.com/enemy-class
...
- Solution: https://tinyurl.com/enemy-class-solution
...
- *Construction*: Constructor runs when an object is declared.
  + Initializes members (optionally via initializer lists).

- *Lifetime (Scope)*: Object exists within its scope.
  + Methods operate on it; =this= refers to the current object.

- *Destruction*: Destructor runs automatically at end of scope.
  + Cleans up resources.
  + Order is LIFO (reverse of construction).

- *RAII Principle*:
  + Resources are acquired in the constructor and released in the destructor.
  + Scope therefore determines resource lifetime.

- *Static Members*:
  + Track or observe object state (e.g., =Player::count=).
  + Independent of any single instance.

- *Scopes as a Tool*:
  + Nested blocks (= { ... } =) reveal when destruction happens.

- *Why It Matters*:
  + Predictable lifetimes → fewer leaks and bugs.
  + Foundation for safe resource management in C++.
