...
* PRACTICE Build a simple calculator (15 min)

- Execute this exercise using the Google Cloud Shell
  ([[https://ide.cloud.google.com][ide.cloud.google.com]]), the ~nano~ editor, and the ~gcc~ compiler. Put
  your result in a file ~calc.c~
...
* PRACTICE Exploring Boolean algebra
...
- Go to CircuitVerse ([[https://circuitverse.org][circuitverse.org]]) and sign up for free with your
  Google Mail account.
...
#+begin_src C :main no :includes :tangle ../src/boolean_solution.c
    ...
    int main() {
      ...
      printf("%d == %d\n", x && y, 0); // 1 && 0 -> TRUE AND FALSE = FALSE
      printf("%d == %d\n", x || y, 1); // 1 || 0 -> TRUE OR  FALSE = TRUE
      printf("%d == %d\n", x == y, 0); // 1 == 0 -> FALSE
      printf("%d == %d\n", !x, 0);     // !1 = 0 -> FALSE
      printf("%d == %d\n", z > x && y < z, 1); // 5>1 && 0<5 ->
                                               // TRUE AND TRUE -> TRUE
...
- How could you show the truth of the equivalence of ~p XOR q~ and ~(p
  ...
  Alternatively, you can do it algebraically using De Morgan Laws (see
  below) and distributive laws.
  #+end_quote
...
n* PRACTICE Logical operators

- Go to Google Cloud shell (ide.cloud.google.com) and run the
  following command on the shell (~-O~ is a big-Oh, not a zero):
  #+begin_example
  wget -O logical.c tinyurl.com/logical-template
  #+end_example
...
* PRACTICE Proving Boolean equivalence with code
...
- Create the input file =truth_table= (or generate it manually on Windoze):
  #+begin_src bash :results output
    echo "0 0" >  truth_table
    echo "0 1" >> truth_table
    echo "1 0" >> truth_table
    echo "1 1" >> truth_table
    cat truth_table
  #+end_src
...
- C code (without loops or arrays)
  #+begin_src C :cmdline < truth_table :main yes :includes <stdio.h> :results output :exports both :noweb yes
...
*equations*. More explicitly: it requires a *set*, some *operations*, and
rules or *axioms* that the operations satisfy (e.g. *commutativity*).
