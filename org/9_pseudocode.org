...
if health < 100  // Logical expression that results in a Boolean value
    Drink a health potion
else
    Go back to fighting
endif
...
4. *Anything Else*: Are there other details in the problem we might need
   ...
   Would anything have changed, if the problem had sounded like this:

   #+begin_quote
   "When a character’s health is 100 or less, drink a health potion
   and feel better instantly. If the health is more than 100, go back
   to fighting, and be fierce."
   #+end_quote
...
- The pseudocode for this problem could look like this:
  ...
  #+begin_example C
  // IF player deposits THEN
  //     add gold to the account
  // ELSE IF player withdraws gold
...
- The former program is available for you to work with at
  [[https://tinyurl.com/pseudogold-solution][tinyurl.com/pseudogold-solution]].
...
#+name: tinyurl.com/pseudogold2-improved
#+begin_src C :cmdline < input :tangle ../src/pseudogold.c
  ...
  int main() {
    ...
    // If (deposit chosen - choice is 1)
    if (choice == 1) {
      // add gold to the account
      gold = gold + amount;
      printf("Deposited %d gold. New balance: %d\n", amount, gold);
    } else {   // Else if (withdrawal chosen - choise is 2)
      // subtract gold from the account
      gold = gold - amount;
      printf("Withdrew %d gold. New balance: %d\n", amount, gold);
    }
...
- So the following two codes are equivalent
  ...
  Algebraically:
  #+begin_example
  If !P is already TRUE, then (!P && Q) = TRUE && Q = Q
  #+end_example
  
* PRACTICE Buy or sell a sword (lab)
...
- The pseudocode has already been copied into the starter code:
  [[https://tinyurl.com/sword-gold-starter][tinyurl.com/sword-gold-starter]]
...
[[https://tinyurl.com/sword-gold-solution][tinyurl.com/sword-gold-solution]]
