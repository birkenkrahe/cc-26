...
- What else might a computer complain about if we tried to run
  ...
  Answer:
  #+begin_quote
  - =health= is undeclared - compiler aborts there.
  #+end_quote
...
- Why is this better?
  ...
  the ~<~ (relational operator), which computers identify as a logical
  expression, which can be evaluated as a *Boolean* value (TRUE/FALSE).
  #+end_quote
...
- Unfortunately, trying to *understand* something can (and perhaps
  should make your brain hurt and may make you feel dizzy for a bit).
...
- *Questions:* What kind of *error handling* might be necessary?
  #+begin_quote
