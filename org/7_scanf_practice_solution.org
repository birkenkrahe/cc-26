...
5) Ask the user for input using the =puts= command.
...
9) If you don't see any output, you might have forgotten to add a
   =printf= command.
...
| 10 | Download the file to PC              | ~Open editor~ - ~Download~             |
    ...
    *Bonus commands:* =history= (of commands =!= to run), =ll= (long listing),
    =ls -lt= (long listing, time-ordered), =clear= (clean up), =*=
    (globbing), =pwd= (present working directory), =whoami= (your
    username), =cd= (change directory), =mkdir= (make directory).
...
#+RESULTS:
: Enter two int and two float values
: |100|-1000|0.456|-934|
...
#+begin_src C :tangle scan2.c :results none
  /******************************************************/
...
n1\frac{d2}{d1 \times d2}+n2\frac{d1}{d2 \times d1} =
\frac{(n1\times d2 + n2\times d1)}{d1\times d2}
$$

Code:
#+begin_src C :cmdline < input4
  /*********************************************************/
