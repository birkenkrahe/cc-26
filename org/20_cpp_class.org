...
- It's just as (deceptively) simple to enter user input in C++:
  Replace the ~cout~ line with the code below and enter some input in
  the ~STDIN~ field.
...
* How to access ~private~ data - =getHealth()=
  ...
  - Let's test it:
    #+begin_src C++ :noweb yes :namespaces std :includes <iostream>
      ...
      cout << "Player's health: " << Jane.getHealth() << endl;
    #+end_src
...
- Example: Create a =Player= that can take damage
  ...
  #+begin_src C++
    ...
    private:
      int health = 100;
    public:
      int getHealth() { return health; } // get Player's health
...
- In the ~main~ program, we're adding a =Player= who can take damage:
  #+begin_src C++ :noweb yes :includes <iostream> :namespaces std :results output
    ...
    cout << "Before the fight: Player's health: "
         << John.getHealth() << endl;

    // Imagine an amazing fight
    John.takeDamage(50);

    cout << "After the fight: Player's health: "
         << John.getHealth();
  #+end_src
...
- Here is the starter code: [[https://onecompiler.com/cpp/43gurdxp8][onecompiler.com/cpp/43gurdxp8]]
  #+begin_src C++ :main no :includes :results output
    #include <iostream> // include input / output
    using namespace std; // use standard names

    /* class definition */
    class Player {// Create a Player class
    private: // private data
    ...
    int main(void) { // BEGIN MAIN
      ...
      cout << "health: " << John.getHealth() << endl;
      // Player takes damage (50)
      John.takeDamage(50);
      // PRINT Player's health after battle + new line
      cout << "health after battle: " << John.getHealth() << endl;
      // Player heals (80)
      John.heal(80);
      // PRINT Player's health after healing + new line
      cout << "health after healing: " << John.getHealth() << endl;
      return 0;
    } // END MAIN
  #+end_src
  ...
  #+RESULTS:
  : health: 100
  : health after battle: 50
  : health after healing: 130
...
- To solve this, go back to your last program and add only one line.
...
- This is the =Player= class so far (without the =health= cap):
  #+begin_src C++ :main yes :results none
    class Player {
    private:
      int health = 100;  // create all players with health 100
    public:
...
- Starter code: [[https://onecompiler.com/cpp/43gpntxa2][onecompiler.com/cpp/43gpntxa2]]
  #+begin_src C++ :main yes :includes :results output
    ...
    { // BEGIN class
    private: // private data:
      int health; // Player's health [initialized outside of class]
    public: // public data
      // Initialize Player with health h
      Player(int h) { // construct Player with health h
        health = h;
      }
      // Return Player's health
      int getHealth() { return health; }
...
- A better style for initializing member variables in constructors is
  ...
  #+begin_src C++ :main no :results output :includes <iostream> :namespaces std
    ...
    int main() {
      Player John(1000); // create a Player with health 
    }
  #+end_src

  #+RESULTS:
  : Player created with health: 1000
...
- Here is the =Player= class with a default destructor method
  ...
  #+begin_src C++ :main yes :noweb yes :namespaces std :includes <iostream> :results none
    ...
    public:
      ...
      // Default destructor
      ~Player() {
        cout << "Game over. Player destroyed. " << endl;
      }
    };
  #+end_src
...
* NEXT Challenge: Create two =Player= characters
...
- Starter code: https://onecompiler.com/cpp/43h2mrae6
  #+begin_src C++ :main no :includes :results output
    #include <iostream>
    using namespace std;
    class Player {
...
- Write a constructor for the =Car= class that creates a =Car= with =speed=
  and =fuel=. When the =Car= ~class~ is used in ~main~ to create a =Car=, it
  should print the message:
  #+begin_example
    Car created with fuel = 100 and speed = 25
  #+end_example

- Constraints:
  1) Initialize the =Car= constructor variables using a list.
  2) Use ~this~ to print =fuel= and =speed= values.
  3) Create a =kia= =Car= object in the ~main~ program. 
...
- The Starter code is here: https://onecompiler.com/cpp/43h48uajv
  ...
  #+begin_src C++ :main no :includes :results output
    ...
    private:
      ...
      int speed;
    public:
      // Construct a Car with `fuel` and `speed`

        // PRINT Car's `fuel` and `speed` values
    ...
    int main(void) { // main program
      // Create a `kia` Car with full fuel tank and 25 mph speed
...
- If we don't get to finish this problem in class, here is a solution:
  [[https://onecompiler.com/cpp/43h5q2mnx][onecompiler.com/cpp/43h5q2mnx]]
