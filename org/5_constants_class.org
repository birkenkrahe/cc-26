...
- [X] Constants with macros or types (=#define=, =const=)
- [X] Library definitions (=#include=)
- [X] Reading input from the keyboard
...
- Can you see what mistake I made in the next code block?[fn:1]
  #+begin_src C :results silent
    #define PI 3.141593  // this is NOT an assignment, it's a macro
    printf("Pi is %f\n", PI);
  #+end_src
...
- Can you see what went wrong in the next code block? If you don't
  ...
  #+begin_src C :results silent
    #define PI 3.141593;  // wrong! it's not a statement!
    printf("PI is %f\n", PI);
  #+end_src
...
- Below, it is included at the start to give us the value of Pi as the
  ...
  #+begin_src C :main no
    #include <stdio.h>
    #include <math.h>  // math functions and definitions available
...
- Modern C has the ~const~ identifier to protect constants. In the code,
  ...
  #+begin_src C
    ...
    printf("Tax on revenue %.2f is %.2f\n", revenue, tax);
  #+end_src
...
- C will not allow you to change the =const= value.
...
*Program template:*
#+begin_src C :tangle constants.c
  ...
  #define EULER M_E
  const int c = 299792458;  // easy to forget the ; here!
...
- Test suite (in OneCompiler, place =100= in the =Input= field):
...
- Use lower case letters for function names
  #+begin_example C
    hello();  // function call without arguments
  #+end_example
...
- Your turn: name some illegal identifiers and see what the compiler
  says!
  #+begin_src C :results silent
    int void = 1;  // attempt to use `void` as a variable name
    float _float = 3.14; // attempt to use `float` as variable name
  #+end_src

- Easy fix:
  #+begin_src C :results silent
    int _void = 1;  // allowed!
    float _float = 3.14; // allowed!
  #+end_src
...
OneCompiler: [[https://onecompiler.com/c/437uq28e8][onecompiler.com/c/437uq28e8]]

Code:
#+begin_src C
// integer constant for the speed of light
  const int c = 299792458; // or use SPEED_OF_LIGHT

  // floating-point constant for pi
  #define PI 3.141593f  // not `pi`

  // integer variable for volume computations
  int volume; // not `v`

  // character variable for last names
  char last_name; // or `lastName`

  // function that adds two integers i and j
  int add_integers(int i,int j) { // not `f` or `add`
    return i + j;
  }

  // variable whose name contains "my", "next", and "birthday"
  int myNextBirthday; // or `my_next_birthday`
#+end_src
