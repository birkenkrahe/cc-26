...
- When you give it a job, the computer needs to carefully *plan* its
  ...
  units of time (hours) but in *bits*.
...
- The price to be paid: To decide how much memory to reserve for its
  operations, you need to assign a *data type* to every number or string
  that you need.

- The prize: Once you've done this (chosen types), you can *change* the
...
=fahrenheit= and =celsius=, and print their values as shown.
...
- In the code block below, the variable =height= gets the value =8= with
  ...
  because it cannot change (it is "literally" the number 8).
  #+begin_example C
    height = 8;
  #+end_example
...
5) To test the code, insert suitable print statements.
...
* PRACTICE In-Class Practice 5: Variable Manipulation and Formatted Output
** Objective
...
Submit your result to Canvas! (=In-class practice 5: Variables=)
