...
- This script introduces C looping structures with ~for~.
...
- The pseudocode for the countdown is here: [[https://tinyurl.com/for-count-up][tinyurl.com/for-count-up]]
  but instead of counting down it counts up from 0 to 9. Turn it into
...
** Solution:
  ...
  1) Pseudocode:
     #+begin_example C
     // DECLARE loop counter i as integer
 
     // FOR i starting at 10 down to 1 DO
...
- The program below can be condensed by converting its ~while~ loop to
  ...
  #+begin_src C :cmdline < ../data/square1_input
    ...
    scanf("%d", &n); printf("%d\n", n);
    // Print loop variable and its square (while loop)
    while (i <= n)
      {
        printf("%10d%10d\n", i, i * i);
        i++;
...
- Solution: ~while~ -> ~for~
  #+begin_src C :cmdline < ../data/square1_input
    ...
    printf("Enter number of entries in table: ");
    scanf("%d", &n); printf("%d\n", n);
    // Print loop variable and its square (for loop)
    for (i = 1; i <= n; i++)
      {
        printf("%10d%10d\n", i, i * i);
      }
