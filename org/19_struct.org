...
- You see that the ~struct~ is just a *data container*. In C++, the
  extension of C with classes, the ~class~ will contain data and
  functions as well - classes are user-defined types.
...
- Really though the ~struct~ is just a warmup for ~class~ in C++ (and
...
- Let's initialize 1000 points & print the last point. Code along:
  ([[https://onecompiler.com/c/43ggz83f6][onecompiler.com/c/43ggz83f6]] - https://tinyurl.com/struct-points
...
Write a short Python program to visualize the 1000 points:
#+begin_src python :file plot1000.png :results file graphics output :exports both :session *Python* :python python3 
  import matplotlib.pyplot as plt

  # generate points
  points = [(i, i*i) for i in range(1000)]
  x = [p[0] for p in points]
  y = [p[1] for p in points]
  # plot
  plt.scatter(x,y,s=5) # s is marker size
  plt.title("1000 points on the graph of f(x) = x^2")
  plt.xlabel("x")
  plt.ylabel("y")
...
- In the following example, we move a point using a pointer by moving
  its x and y coordinates.

- Example: code along at [[https://onecompiler.com/c/43gh38ut7][onecompiler.com/c/43gh38ut7]] or
  https://tinyurl.com/struct-pointer
...
1) In data science, you often work with structured data (e.g. records
   ...
   dictionary in Python, or data.frame in R, lets you group related
...
DSC 105 (Introduction to data science), or CSC 240 (Data Structures
with C++)!
